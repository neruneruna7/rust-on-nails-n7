    ...
    B::Future: Send + 'static,
{
    ...
    fn poll_ready(&mut self, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        ...
        // 各内部サービスの準備状況を駆動し、準備ができているレコードを記録します
        loop {
    ...
    fn call(&mut self, req: Request<Body>) -> Self::Future {
        ...
        // ユーザーに最初に `poll_ready` を呼び出すように要求します。呼び出さなかった場合、パニックが発生する可能性があります
        // `tower::Service` コントラクトに従って
        assert!(
            self.grpc_ready,
        ...
        // grpc リクエストを受け取った場合は grpc サービスを呼び出し、それ以外の場合はrest サービスを呼び出します
        // サービスを呼び出すと、サービスが準備完了になっていないため、ドライブの準備が再び整います
