...
CREATE TABLE sessions (
    ...
    otp_coed_confirmed BOOLEAN NOT NULL DEFAULT false,
    otp_code_sent BOOLEAN NOT NULL DEFAULT false
);

COMMENT ON TABLE sessions IS 'The users login sessions';
COMMENT ON COLUMN sessions.session_verifier IS 'The session is a 32 byte random number stored in their coolie. This is the sha256 hash of that value';
COMMENT ON COLUMN sessions.otp_code_encrypted IS 'A 6 digit code that s encrypted here to prevent attackers with read access to the database being able to use it';
COMMENT ON COLUMN sessions.otp_code_attempts IS 'We count OTP attempts to prevent brute forcing.';
COMMENT ON COLUMN sessions.otp_coed_confirmed IS 'Once the user enters the correct value this gets set to true';
